...
export default function CineCityFinal() {
  ...
  return (
    <div className={`w-full min-h-screen bg-white text-zinc-900 transition-all duration-1000 selection:bg-orange-500 selection:text-white overflow-x-hidden ${isAltVersion ? 'font-serif' : 'font-sans'}`}>
      ...
      {/* 1. LAYOUT SIDE-BAR (Desktop Only) */}
      <motion.div 
        initial={{ x: 100 }}
        animate={{ x: 0 }}
      ...
      {/* 2. AUTOMATISCHE MARQUEE - Responsive Text */}
      <div className="pt-24 overflow-hidden whitespace-nowrap border-b border-zinc-100 pb-4">
        ...
        >
          {[...Array(15)].map((_, i) => (
            <span key={i} className={`text-[min(3vw,14px)] font-black uppercase tracking-[0.4em] md:tracking-[0.6em] opacity-20 ${isAltVersion ? 'italic text-orange-600' : 'text-blue-600'}`}>
      ...
      >
        ...
        <span className="text-[9px] md:text-[10px] font-black uppercase tracking-widest">Toggle Identity</span>
      </button>
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-12 pt-12 md:pt-20 pb-8 md:pb-16">
        <div className="mb-6 md:mb-10">
          <motion.h1 
            layout
            className={`
              /* Fluid Text: van ~60px op mobiel naar ~160px op desktop */
              text-[clamp(3.5rem,14vw,10.5rem)] 
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-t border-zinc-100 pt-8">
          <div className="flex flex-col gap-2">
            <span className="text-[10px] md:text-xs font-bold uppercase tracking-[0.2em] text-zinc-400">Identity Concept</span>
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-12 py-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 lg:gap-8 auto-rows-[280px] md:auto-rows-[350px]">
          {gridItems.map((item, index) => (
            <motion.div
              ...
              className={`group relative cursor-pointer overflow-hidden border border-zinc-100 transition-all duration-700
                ${item.size} 
                ${isAltVersion ? 'rounded-none border-zinc-300 p-2 bg-zinc-50' : 'rounded-[2rem] lg:rounded-[4rem] shadow-sm bg-white'}`}
            >
      ...
      <footer className="max-w-7xl mx-auto px-6 md:px-12 py-20 md:py-32 border-t border-zinc-100 mt-20">
        <div className="flex flex-col md:flex-row justify-between items-center gap-12 text-center md:text-left">
          <div className="space-y-2">
             <h2 className="text-3xl md:text-5xl font-black uppercase tracking-tighter">Visual Identity</h2>
             <p className="text-xs md:text-sm text-zinc-400 uppercase tracking-[0.3em] italic">CineCity / Antwerp / 2025</p>
          </div>
          ...
          >
            <span className="text-xs font-black uppercase tracking-[0.2em]">Ontdek meer</span>
        ...
        {selectedImg !== null && (
          ...
          >
            ...
            >
              <img 
                src={currentItem?.img} 
                className={`w-full h-full max-h-[80vh] object-contain shadow-2xl ${isAltVersion ? 'rounded-none' : 'rounded-2xl lg:rounded-[3rem]'}`} 
              />
            </motion.div>
          </motion.div>
        )}
